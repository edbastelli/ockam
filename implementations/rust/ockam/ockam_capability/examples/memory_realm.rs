...
use ockam_capability::{
    MyCapabilities, MyCapability, MyCapabilityRequest, MyCapabilityResponse,
    MyUniqueUnforgeableReference,
};
...
pub trait Behaviour {
    ...
    async fn handle_message(
        &mut self,
        ctx: &mut Context,
        msg: Routed<MyCapabilityRequest>,
        capable: &mut Capable,
    ) -> ockam::Result<()> {
        Ok(())
    }
...
pub struct Capable {
    ...
    uur: MyUniqueUnforgeableReference,
}

impl Capable {
    pub fn with_capabilities(name: &str, capabilities: MyCapabilities) -> Self {
        let this = Self {
            name: name.to_string(),
            capabilities,
            uur: MyUniqueUnforgeableReference(random()),
        };
        info!("{}'s uur is:\t{:?}", name, this.uur);
        this
    }

    pub fn my_capability(&self) -> MyCapability {
        MyCapability {
    ...
    pub fn introduce_me(&mut self, name: &'static str, capability: MyCapability) {
        self.capabilities.insert(name, capability);
    }

    pub fn can_delegate(&self, name: &str) -> Option<MyCapability> {
    ...
    pub fn approves(&self, capability: &MyCapability) -> bool {
...
impl<B> CapableWorker<B> {
    ...
    fn my_capability(&self) -> MyCapability {
    ...
    B: Behaviour + Send + Sync + 'static,
{
    ...
    type Message = MyCapabilityRequest;
...
async fn main(ctx: Context) -> ockam::Result<()> {
    ...
    let alice = CapableWorker::with_capabilities(
        "Alice",
        Alice {},
        MyCapabilities::from_iter([("cap_bob", bob.my_capability())]),
    );
...
impl Behaviour for Alice {
    ...
    async fn handle_message(
        ...
        if let MyCapabilityRequest::IntroduceMe(sender_capability, requested_capability_name) =
            &msg.body()
        {
            info!(
            ...
            if !capable_alice.approves(sender_capability) {
                ...
                return ctx
                    .send(return_route.clone(), MyCapabilityResponse::Unauthorized)
                    .await;
            }
            ...
            if let Some(capability) = capable_alice.can_delegate(requested_capability_name.as_str())
            {
                ...
                let cap_response = MyCapabilityResponse::Introduction(capability.clone());
                return ctx.send(return_route.clone(), cap_response).await;
            }
        }

        ctx.send(return_route.clone(), MyCapabilityResponse::Unauthorized)
            .await
    }
}
...
impl Behaviour for Bob {
    ...
    async fn handle_message(
        ...
        if let MyCapabilityRequest::OhHaiBob(some_capability) = &msg.body() {
            ...
            let cap_response = MyCapabilityResponse::OhHaiCarol;
            return ctx.send(return_route.clone(), cap_response).await;
        }
...
impl Behaviour for Carol {
    async fn initialize(
        ...
        if let Some(cap_alice) = capable_carol.can_delegate("cap_alice") {
            ...
            let cap_request =
                MyCapabilityRequest::IntroduceMe(cap_alice.clone(), "cap_bob".to_string());
            ctx.send("alice_address", cap_request).await?;

            let response = ctx.receive::<MyCapabilityResponse>().await?;
            ...
            if let MyCapabilityResponse::Introduction(cap_bob) = response {
                info!("[Carol] Carol can now try saying OhHai to Bob");
                ctx.send("bob_address", MyCapabilityRequest::OhHaiBob(cap_bob))
                    .await?;

                let oh_hai = ctx.receive::<MyCapabilityResponse>().await?;
                let oh_hai = oh_hai.take().body();
                if let MyCapabilityResponse::OhHaiCarol = oh_hai {
                    info!("");
